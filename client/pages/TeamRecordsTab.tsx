      ...
      <div className="flex flex-col gap-4 animate-fade-in mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          ...
          <div className="flex bg-slate-900/80 rounded-xl p-1 border border-white/5 shadow-inner">
            <button onClick={() => setSortField('played')}
                    className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${sortField === 'played' ? 'bg-brand-500 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>Most
              Played
            </button>
            <button onClick={() => setSortField('won')}
                    className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${sortField === 'won' ? 'bg-brand-500 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>Most
              Wins
            </button>
            <button onClick={() => setSortField('winrate')}
                    className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${sortField === 'winrate' ? 'bg-brand-500 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>Win
              Rate
            </button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto relative z-10">
            <table className="w-full text-left border-collapse">
              ...
              {sortedRecords.map((rec, i) => {
                ...
                return (
                    <tr key={i} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex flex-wrap gap-2">
                          {rec.teamMembers.map((name, j) => (
                              <span key={j}
                                    className="px-3 py-1 bg-white/5 rounded text-sm text-slate-200 border border-white/10">
                            {name}
                          </span>
                          ))}
                        </div>
                      ...
                      <span
                          className={`inline-flex px-2.5 py-1 rounded-lg text-sm font-bold shadow-inner ${winRate >= 50 ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                        {winRate.toFixed(1)}%
                      </span>
                      </td>
                    </tr>
              ...
              {sortedRecords.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-slate-400">No team records
                      found.
                    </td>
                  </tr>
              )}
