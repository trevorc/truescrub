...
def test_skill_groups() -> None:
    """Test the skill group index calculations."""
    assert Player(1, "", 0, 125, 0.0).skill_group_index == 0
    assert Player(1, "", -1337, 125, 0.0).skill_group_index == 0
    assert Player(1, "", sys.maxsize, 25, 0.0).skill_group_index == len(
        SKILL_GROUP_NAMES
    )
...
def test_player() -> None:
    ...
    mean, stdev = 1379, 76
    player = Player(
        player_id=76561198121510237,
        steam_name="nonverba1",
        skill_mean=mean,
        skill_stdev=stdev,
        impact_rating=1.02,
    )
