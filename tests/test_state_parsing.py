...
from truescrub.statewriter.state_parsing import (
  DeserializationError, InvalidGameStateException, parse_game_state, parse_map,
  parse_weapon, parse_weapons)
...
def test_parse_weapon():
  ...
  weapon_proto = parse_weapon(weapon_json)
  expected_proto = game_state_pb2.Weapon(
    name="weapon_ak47",
    paintkit="cu_ak47_mastery",
    type=game_state_pb2.Weapon.WEAPON_TYPE_RIFLE,
    ammo_clip=30,
    ammo_clip_max=30,
    ammo_reserve=0,
    active=True,
  ...
  assert text_format.MessageToString(
    weapon_proto) == text_format.MessageToString(expected_proto)

  weapon_json_holstered = {
    "name": "weapon_knife",
    "paintkit": "default",
    "type": "Knife",
    "state": "holstered"
  }
  weapon_proto_holstered = parse_weapon(weapon_json_holstered)
  expected_proto_holstered = game_state_pb2.Weapon(
    name="weapon_knife",
    paintkit="default",
    type=game_state_pb2.Weapon.WEAPON_TYPE_KNIFE,
    active=False
  )
  assert (text_format.MessageToString(weapon_proto_holstered) ==
          text_format.MessageToString(expected_proto_holstered))

  weapon_json_unknown_type = {
    "name": "weapon_unknown",
    "type": "Unknown",
    "state": "active"
  }
  with pytest.raises(DeserializationError):
    parse_weapon(weapon_json_unknown_type)


def test_parse_weapons():
  weapons_json = {
    "weapon_0": {
      "name": "weapon_knife",
      "paintkit": "default",
  ...
  weapons_proto = parse_weapons(weapons_json)
  expected_proto = game_state_pb2.Weapons(
    weapon_0=game_state_pb2.Weapon(
      name="weapon_knife",
      paintkit="default",
      type=game_state_pb2.Weapon.WEAPON_TYPE_KNIFE,
      active=False,
    ),
    weapon_1=game_state_pb2.Weapon(
      name="weapon_glock",
      paintkit="default",
      type=game_state_pb2.Weapon.WEAPON_TYPE_PISTOL,
      ammo_clip=20,
      ammo_clip_max=20,
      ammo_reserve=120,
      active=False,
  ...
  assert text_format.MessageToString(
    weapons_proto) == text_format.MessageToString(expected_proto)
